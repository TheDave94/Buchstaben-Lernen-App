...
|---|------|------|---------------|
| H1 | `PilotAudioCondition: { phoneme, arbitrarySound, silent }` enum + assignment. Pedagogical flow held constant (D1), so this is the audio dimension only. **SHIPPED (`279d553`, 2026-06-20)** — orthogonal to `ThesisCondition` (UUID byte 15, decorrelated from byte 0), with override, enrollment, and exporter stamping; `PilotAudioConditionAssignmentTests`. | Build | Slotted into existing UUID-modulo assignment + override + enrollment + per-arm exporter, as planned. |
| H2 | Arm-aware audio selection — `activeAudioFiles(for:)` branches on the pilot arm. **SHIPPED (`b212e82`, 2026-06-20)** — `.silent → []`, `.arbitrarySound → asset.arbitraryAudioFiles` (empty placeholder until H4/D2, deliberately no name-audio fallback), `.phoneme → phoneme/name per toggle` (H2.1 tightened this; see known issues); `AudioArmRoutingTests`. | Build | Single chokepoint as planned. Both audio arms keep the identical shared `setAdaptivePlayback` coupling — the coupling path reads no arm state, only the file list differs (§2.6 matching discipline). |
