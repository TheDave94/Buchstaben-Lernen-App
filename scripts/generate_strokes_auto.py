...
LETTERS: dict[str, list[StrokeSpec]] = {
    ...
    "D": [
        ...
        # left via STEM_CENTER. Bowl reuses the stem's endpoints
        # (STEM_CENTER_T / STEM_CENTER_B) so the bowl polyline starts
        # at the same pixel as the stem polyline at the top and ends
        # at the same pixel at the bottom — visibly meeting on stem.
        {"kind": "line",
         "anchors": ["STEM_CENTER_T", "STEM_CENTER_B"],
         "arms": ["straight_line"]},
        {"kind": "line",
         "anchors": ["STEM_CENTER_T", "RIGHT_MID", "STEM_CENTER_B"],
         "arms": ["smoothed_medial_axis", "smoothed_medial_axis"]},
    ],
    "B": [
...
STEM_CAP_OFFSET = 8  # px in from literal cap/foot apex
    ...
    for i, spec in enumerate(specs, start=1):
        ...
        if kind == "line":
            ...
            for k in range(n_arms):
                ...
                # literal resolved anchor pixel so connecting strokes
                # (stem + bowl, bowl + leg) share their endpoint
                # exactly. The BFS used the skeleton snap only as a
                # starting point — the polyline endpoint is the
                # anchor verbatim.
                if arms[-1] is not None:
                    p = arms[-1]
                    if names[k] in STEM_MEETING_ANCHORS:
                        p = [(float(anchors[k][0]), float(anchors[k][1]))] + list(p[1:])
                    if names[k + 1] in STEM_MEETING_ANCHORS:
                        p = list(p[:-1]) + [(float(anchors[k + 1][0]),
                                              float(anchors[k + 1][1]))]
                    arms[-1] = p
